...
class Slider extends Component {
  ...
  _onMouseDrag(mouseX, mouseY, mouseDownX, mouseDownY) {
    ...
    let value = Math.max(
      0.0,
      Math.min(1.0, this._valueAtDragStart + dm * sensitivity)
    );
  ...
  _renderVectorGraphics(value, width, height) {
    ...
    const outerCircle = new Paper.Path.Circle(new Paper.Point(cx, cy), radius);
    const innerCircle = new Paper.Path.Circle(new Paper.Point(cx, cy), radius);
    outerCircle.rotate(90);
    innerCircle.rotate(90);
    return `
      <svg
        width="${width}"
        height="${height}"
